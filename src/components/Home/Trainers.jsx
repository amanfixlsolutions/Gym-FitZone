...
const FALLBACK_IMAGE = "https://images.unsplash.com/photo-1581009137042-c552e485697a?w=400&h=400&fit=crop";
...
const Trainers = () => {
  ...
  useEffect(() => {
    const fetchTrainers = async () => {
      try {
        const res = await trainerAPI.getPublic({ status: "Active", limit: 6 });
        const raw = res.data || res.trainers || [];
        setTrainers(raw);
      } catch {
        setTrainers([]);
      } finally {
  ...
  const maxIndex = Math.max(0, trainers.length - slidesToShow);
  ...
  const startAutoSlide = () => {
    ...
    autoSlideInterval.current = setInterval(nextSlide, 2500);
  };
  ...
  const openTrainerDetails = (trainer) => setSelectedTrainer(trainer);
  const closeTrainerDetails = () => setSelectedTrainer(null);
  ...
  // ── Normalize backend trainer data ─────────────────────────────
  const normalizeTrainer = (t) => ({
    _id:          t._id,
    name:         t.name || "Trainer",
    role:         t.specialty || t.specialization || t.role || "Fitness Trainer",
    image:        resolvePhoto(t.photo || t.image) || FALLBACK_IMAGE,
    experience:   t.experience ? `${t.experience}+ years` : "5+ years",
    achievements: t.certifications?.[0] || t.achievement || "Certified Trainer",
    rating:       t.rating || 4.8,
    specialties:  t.specialties || (t.specialty ? [t.specialty] : ["Fitness Training"]),
    education:    t.education || "Fitness Certification",
    certification:t.certifications?.[0] || t.certification || "Certified Trainer",
    clients:      t.clientsCount ? `${t.clientsCount}+` : "100+",
    facebook:     t.socialLinks?.facebook || t.facebook || null,
    instagram:    t.socialLinks?.instagram || t.instagram || null,
    twitter:      t.socialLinks?.twitter || t.twitter || null,
    bio:          t.bio || t.description || `${t.name || "This trainer"} is a certified fitness professional dedicated to helping clients achieve their goals.`,
  });
            ...
            <>
              <div className="relative group">
                <div className="overflow-hidden rounded-xl">
                  ...
                  >
                    {trainers.map((rawTrainer, index) => {
                      const trainer = normalizeTrainer(rawTrainer);
                      return (
                        <div
            ...
            <div className="relative">
              ...
              <div className="flex flex-col md:flex-row">
                ...
                <div className="md:w-3/5 p-5 md:p-8">
                  ...
                  <a href="/membership" className="mt-4 md:mt-6 block w-full py-2 md:py-2.5 bg-gradient-to-r from-amber-500 to-orange-500 text-white text-xs md:text-sm font-semibold rounded-lg hover:shadow-lg transition-all duration-300 text-center">
