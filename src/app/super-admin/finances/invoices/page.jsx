...
import { Download, Search, RefreshCw, Send, FileText, ChevronDown } from "lucide-react";
...
export default function Page() {
  ...
  const filtered = invoices.filter(inv => {
    ...
    return inv.invoiceNumber?.toLowerCase().includes(q) || inv.memberName?.toLowerCase().includes(q) || inv.gymName?.toLowerCase().includes(q) || inv.planName?.toLowerCase().includes(q);
  });

  const handleExport = () => {
    if (!filtered.length) return;
    const headers = ["Invoice #", "Member", "Gym", "Plan", "Total", "Status", "Date"];
    const rows = filtered.map(inv => [inv.invoiceNumber, inv.memberName, inv.gymName || "", inv.planName || "", inv.total, inv.status, fmtDate(inv.createdAt)]);
  ...
  return (
    <RoleDashboardLayout title="Invoices" navItems={SUPER_ADMIN_NAV} role="super-admin">
      <div className="space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-[var(--text)]">Platform Invoices</h1>
            <p className="text-sm text-[var(--muted)] mt-0.5">All membership invoices — {totalCount.toLocaleString()} total</p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1 relative">
            ...
            <input type="text" placeholder="Search by invoice #, member, gym..."
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[700px]">
              <thead className="bg-[var(--surface2)] border-b border-[var(--border)]">
                <tr>
                  {["Invoice #", "Member", "Gym", "Plan", "Amount", "Issue Date", "Due Date", "Status", "Actions"].map(h => (
                ...
                {loading ? (
                  [...Array(8)].map((_, i) => (
                    <tr key={i} className="animate-pulse">
                      {[...Array(9)].map((_, j) => <td key={j} className="px-4 py-3"><div className="h-3 bg-[var(--surface2)] rounded w-20" /></td>)}
                    </tr>
                  ))
                ) : filtered.length === 0 ? (
                  ...
                  <tr key={inv._id} className="hover:bg-[var(--surface2)] transition-colors">
                    <td className="px-4 py-3 font-mono text-xs text-violet-600 font-semibold whitespace-nowrap">{inv.invoiceNumber}</td>
                    <td className="px-4 py-3 font-medium text-[var(--text)] whitespace-nowrap">{inv.memberName}</td>
                    <td className="px-4 py-3 text-[var(--muted)] whitespace-nowrap">{inv.gymName || "—"}</td>
                    ...
                    <td className="px-4 py-3">
                      {inv.status !== "Paid" && inv.status !== "Cancelled" && (
                        <button onClick={() => handleSend(inv)} disabled={sending === inv._id}
                          className="p-1.5 hover:bg-violet-50 dark:hover:bg-violet-900/20 rounded-lg cursor-pointer" title="Send Invoice">
                          {sending === inv._id
                            ? <span className="w-3.5 h-3.5 border border-violet-400/40 border-t-violet-500 rounded-full animate-spin inline-block" />
