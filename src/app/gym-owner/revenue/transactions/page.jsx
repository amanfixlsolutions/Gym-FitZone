...
import { Search, Download, RefreshCw, TrendingUp, TrendingDown, ChevronDown } from "lucide-react";
...
export default function Page() {
  ...
  const filtered = payments.filter(p => {
    ...
    return p.memberName?.toLowerCase().includes(q) || p.planName?.toLowerCase().includes(q) || p.gateway?.toLowerCase().includes(q);
  });
  ...
  const handleExport = () => {
    ...
    const headers = ["Member", "Plan", "Amount", "Gateway", "Type", "Status", "Date"];
    const rows = filtered.map(p => [p.memberName, p.planName || "", p.amount, p.gateway, p.type, p.status, fmtDate(p.createdAt)]);
  ...
  return (
    <RoleDashboardLayout title="Transactions" navItems={GYM_OWNER_NAV} role="gym-owner">
      <div className="space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-[var(--text)]">Transactions</h1>
            <p className="text-sm text-[var(--muted)] mt-0.5">All payment records — {totalCount.toLocaleString()} total</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { label: "This Month",      value: fmtMoney(revenue.monthly), icon: TrendingUp,   color: "blue" },
            { label: "Total Refunds",   value: fmtMoney(totalRefunds),    icon: TrendingDown, color: "red" },
            { label: "Failed",          value: fmtMoney(totalFailed),     icon: TrendingDown, color: "amber" },
          ].map((s) => (
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1 relative">
            ...
            <input type="text" placeholder="Search by member, plan..."
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[600px]">
              <thead className="bg-[var(--surface2)] border-b border-[var(--border)]">
                <tr>
                  {["Txn ID", "Member", "Plan", "Amount", "Gateway", "Type", "Date", "Status"].map(h => (
                ...
                {loading ? (
                  [...Array(8)].map((_, i) => (
                    <tr key={i} className="animate-pulse">
                      {[...Array(8)].map((_, j) => <td key={j} className="px-4 py-3"><div className="h-3 bg-[var(--surface2)] rounded w-16" /></td>)}
                    </tr>
                  ))
                ) : filtered.length === 0 ? (
                  <tr><td colSpan={8} className="px-4 py-10 text-center text-sm text-[var(--muted)]">No transactions found</td></tr>
                ) : filtered.map(p => (
