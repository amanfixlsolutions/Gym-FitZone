...
const INITIAL_PLANS = [
  ...
  { id: "P002", name: "Monthly",    price: 1499,   duration: "30",   unit: "Days",   features: ["Full gym access", "Locker room", "1 PT session"],                    color: "blue",    active: 4200 },
  { id: "P003", name: "Quarterly",  price: 3999,   duration: "90",   unit: "Days",   features: ["Full gym access", "Locker room", "3 PT sessions", "Diet plan"],      color: "purple",  active: 2800, popular: true },
  { id: "P004", name: "Yearly",     price: 11999,  duration: "365",  unit: "Days",   features: ["Full gym access", "Locker room", "Unlimited PT", "Diet plan", "Priority booking"], color: "emerald", active: 1900 },
  { id: "P005", name: "Class Pass", price: 2999,   duration: "30",   unit: "Days",   features: ["Unlimited classes", "Yoga, Zumba, HIIT", "App access"],              color: "amber",   active: 600 },
];
...
const EMPTY_FORM = { name: "", price: "", duration: "", unit: "Days", color: "blue", features: [""], popular: false };
...
export default function Page() {
  ...
  const openAdd = () => { setEditPlan(null); setForm(EMPTY_FORM); setShowModal(true); };
  const openEdit = (plan) => {
    setEditPlan(plan);
  ...
  return (
    <RoleDashboardLayout title="Plans & Pricing" navItems={GYM_OWNER_NAV} role="gym-owner" userName="Suresh Nair" userEmail="suresh@ironparadise.in" userAvatar="SN">
      <div className="space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-[var(--text)]">Plans & Pricing</h1>
            <p className="text-sm text-[var(--muted)] mt-0.5">Manage membership plans for Iron Paradise</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
          {plans.map(plan => {
            ...
            return (
              <div key={plan.id} className={`bg-[var(--surface)] border-2 rounded-2xl p-5 sm:p-6 relative hover:shadow-lg transition-shadow ${plan.popular ? "border-purple-500" : "border-[var(--border)]"}`}>
                {plan.popular && <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-purple-600 text-white text-[10px] font-bold px-3 py-1 rounded-full whitespace-nowrap">MOST POPULAR</span>}
          ...
          <div className="bg-[var(--surface)] rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            {success ? (
              <div className="flex flex-col items-center justify-center py-12 px-6">
                ...
                <p className="text-sm text-[var(--muted)] mt-1">{form.name} plan has been {editPlan ? "updated" : "created"} successfully.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
                ...
                <div className="flex items-center justify-between py-3 border border-[var(--border)] rounded-xl px-4">
                  <div>
                    ...
                    <p className="text-xs text-[var(--muted)] mt-0.5">Shows a "Most Popular" badge on this plan</p>
                  </div>
