        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-5xl mx-auto">
            ...
            <div className="bg-white rounded-2xl p-6 md:p-8 shadow-md">
              ...
              <p className="text-xs text-black mb-5">We'll get back to you within 24 hours.</p>
              ...
              {submitted ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input required type="text" value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="Rahul Sharma"
                        className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition-all" />
                    </div>
                    <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-black mb-1">Subject *</label>
                      <select required value={form.subject} onChange={e => setForm({...form, subject: e.target.value})}
                        className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition-all text-gray-600">
                  ...
                  <div>
                    ...
                    <textarea required rows={4} value={form.message} onChange={e => setForm({...form, message: e.target.value})} placeholder="Tell us how we can help you..."
                      className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition-all resize-none" />
                  </div>
                  <button type="submit"
              ...
              <div className="bg-white rounded-2xl overflow-hidden shadow-md h-48 relative">
                <div className="absolute inset-0 bg-gradient-to-br from-amber-100 to-orange-100 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-sm font-semibold text-black">Iron Paradise Fitness</p>
                    <p className="text-xs text-black">Andheri West, Mumbai</p>
                ...
                <div className="flex gap-3">
                  {socialLinks.map((s, i) => (
                    <a key={i} href={s.href} target="_blank" rel="noopener noreferrer"
                      className={`w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center text-gray-500 hover:text-white transition-all duration-300 hover:scale-110 ${s.color}`}
                      aria-label={s.label}>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[["View Classes", "/user/classes"], ["Meet Trainers", "/user/trainers"], ["Membership Plans", "/user/membership"], ["Book a Trial", "/user/membership"]].map(([label, href], i) => (
                    <Link key={i} href={href}
                      className="flex items-center gap-1.5 text-xs text-black hover:text-amber-500 transition-colors group">
