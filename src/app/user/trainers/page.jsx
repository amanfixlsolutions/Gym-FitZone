    ...
    <div className="bg-white">
      <section className="pt-24 pb-10 bg-white">
        <div className="container mx-auto px-4 text-center">
          ...
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-gray-800 mt-2 mb-4">Meet Our <span className="text-amber-500">Expert Trainers</span></h1>
          <div className="w-16 h-0.5 bg-amber-500 mx-auto mb-4" />
          <p className="text-gray-500 text-sm md:text-base max-w-xl mx-auto">Learn from the best certified fitness professionals in the industry.</p>
        </div>
      </section>

      {/* Slider */}
      <section className="py-8 bg-gradient-to-br from-amber-50 to-white">
        <div className="container mx-auto px-4">
          <h2 className="text-xl md:text-2xl font-bold text-gray-800 mb-6">Featured Trainers</h2>
                ...
                {trainers.map((t, i) => (
                  <div key={i} className="flex-shrink-0 px-2" style={{ width: `${100 / slidesToShow}%` }}>
                    <div className="bg-gradient-to-br from-amber-50 to-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 hover:scale-[1.02] cursor-pointer" onClick={() => setSelected(t)}>
                      <div className="relative"><div className="relative w-full pt-[100%]"><img src={t.image} alt={t.name} className="absolute top-0 left-0 w-full h-full object-cover hover:scale-110 transition-transform duration-500" /></div>
                      ...
                      <div className="p-3">
                        ...
                        <div className="flex gap-2 justify-center mt-2.5">
                          {[FaFacebook, FaInstagram, FaTwitter].map((Icon, j) => (
                            <button key={j} onClick={(e) => openLink("https://example.com", e)} className="w-6 h-6 bg-amber-100 rounded-full flex items-center justify-center hover:bg-gradient-to-r hover:from-amber-500 hover:to-orange-500 hover:text-white transition-all duration-300 hover:scale-110 text-amber-600"><Icon className="w-3 h-3" /></button>
                          ))}
                        </div>
                      </div>
                    </div>
      ...
      <section className="py-10 bg-white">
        <div className="container mx-auto px-4">
          <h2 className="text-xl md:text-2xl font-bold text-gray-800 mb-2">All Trainers</h2>
          <p className="text-xs text-gray-400 mb-6">Showing {paged.length} of {trainers.length} trainers · Click a card to view full profile</p>
            ...
            {paged.map((t) => (
              <div key={t.id} className="bg-gradient-to-br from-amber-50 to-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 hover:scale-[1.02] cursor-pointer" onClick={() => setSelected(t)}>
                <div className="flex items-center gap-4 p-5">
                  <div className="relative flex-shrink-0"><img src={t.image} alt={t.name} className="w-20 h-20 rounded-xl object-cover" /><div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white ${t.available ? "bg-emerald-500" : "bg-red-400"}`} /></div>
          ...
          <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex flex-col md:flex-row">
              <div className="md:w-2/5 bg-gradient-to-br from-amber-500 to-orange-500 p-6 md:p-8 flex flex-col items-center justify-center">
                <div className="w-32 h-32 md:w-44 md:h-44 rounded-full overflow-hidden border-4 border-white shadow-xl"><img src={selected.image} alt={selected.name} className="w-full h-full object-cover" /></div>
