...
const plans = [
  { name: "Basic", price: "$20", period: "/mo", icon: "🎯", features: ["Gym Access (No Trainer)", "Unlimited Access", "Access to all Clubs", "Training for all Classes", "Exclusive Studio", "Additional Session"], popular: false },
  { name: "Standard", price: "$35", period: "/mo", icon: "⭐", features: ["Gym Access + 1 Trainer", "Unlimited Access", "Access to all Clubs", "Training for all Classes", "Exclusive Studio", "Additional Session"], popular: false },
  { name: "Professional", price: "$50", period: "/mo", icon: "👑", features: ["Gym + 3 Trainer Sessions", "Unlimited Access", "Access to all Clubs", "Training for all Classes", "Exclusive Studio", "Additional Session"], popular: true },
  { name: "Family", price: "$65", period: "/mo", icon: "🏠", features: ["Up to 4 Members", "Unlimited Access", "Access to all Clubs", "Training for all Classes", "Exclusive Studio", "Additional Session"], popular: false },
];
    ...
    <div className="bg-white">
      <section className="min-h-screen flex flex-col justify-center bg-gradient-to-br from-amber-50 to-orange-50 pt-16">
        <div className="container mx-auto px-4 py-10">
          <div className="text-center mb-6">
            ...
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-gray-800 mb-3">Membership <span className="text-amber-500">Plans</span></h1>
            <div className="w-16 h-0.5 bg-amber-500 mx-auto mb-4" />
            <p className="text-gray-600 text-sm md:text-base max-w-xl mx-auto mb-5">Choose the plan that fits your goals. No hidden fees, cancel anytime.</p>
              ...
              <div key={i} className={`bg-white rounded-2xl shadow-md overflow-hidden transform transition-all duration-300 hover:scale-105 hover:shadow-xl cursor-pointer relative ${plan.popular ? "border-2 border-amber-500 shadow-lg" : "border border-gray-100"}`}>
                ...
                <div className="p-5">
                  ...
                  <div className="mt-2 mb-4">
                    <span className="text-3xl font-bold text-gray-800">{billingCycle === "yearly" ? `$${Math.round(parseInt(plan.price.replace("$", "")) * 0.8)}` : plan.price}</span>
                    <span className="text-gray-500 text-sm">{billingCycle === "yearly" ? "/mo (billed yearly)" : plan.period}</span>
                  </div>
                  <ul className="space-y-2 mb-5">
                    {plan.features.map((f, j) => (
                      <li key={j} className="flex items-center gap-2 text-gray-600 text-xs"><CheckCircle className="w-3.5 h-3.5 text-amber-500 flex-shrink-0" />{f}</li>
                    ))}
                  </ul>
                  <button className={`w-full py-2.5 rounded-full font-semibold text-sm transition-all duration-300 hover:scale-105 active:scale-95 ${plan.popular ? "bg-gradient-to-r from-amber-500 to-orange-500 text-white hover:shadow-lg" : "border border-amber-500 text-amber-600 hover:bg-amber-500 hover:text-white"}`}>Join Now</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="min-h-screen flex flex-col justify-center bg-white py-10">
        <div className="container mx-auto px-4">
          <div className="text-center mb-6">
            <h2 className="text-2xl md:text-3xl font-bold text-gray-800">Compare <span className="text-amber-500">Plans</span></h2>
          ...
          <div className="overflow-x-auto">
            <table className="w-full max-w-4xl mx-auto text-sm">
              ...
              <tbody>
                {[["Gym Access", true, true, true, true], ["Personal Trainer", false, "1x/mo", "3x/mo", "Unlimited"], ["Group Classes", true, true, true, true], ["Nutrition Plan", false, false, true, true], ["Family Members", "1", "1", "1", "Up to 4"], ["Spa Access", false, false, true, true], ["Priority Booking", false, false, true, true]].map(([feature, ...vals], i) => (
                  <tr key={i} className={`border-b border-gray-50 ${i % 2 === 0 ? "bg-amber-50/30" : ""}`}>
                    ...
                    {vals.map((v, j) => <td key={j} className="py-3 px-4 text-center">{v === true ? <CheckCircle className="w-4 h-4 text-amber-500 mx-auto" /> : v === false ? <span className="text-gray-300 text-lg">—</span> : <span className="text-xs font-semibold text-gray-700">{v}</span>}</td>)}
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="mt-10">
            <div className="text-center mb-5"><h3 className="text-xl font-bold text-gray-800">What <span className="text-amber-500">Members Say</span></h3><div className="w-12 h-0.5 bg-amber-500 mx-auto mt-2" /></div>
      ...
      <section className="min-h-screen flex flex-col justify-center bg-gradient-to-br from-amber-50 to-orange-50 py-10">
        <div className="container mx-auto px-4">
          ...
          <div className="bg-gradient-to-r from-amber-500 to-orange-500 rounded-3xl p-8 md:p-12 text-center max-w-3xl mx-auto shadow-xl">
            ...
            <button className="bg-white text-amber-600 px-8 py-3 rounded-full font-bold text-sm hover:shadow-xl transition-all duration-300 hover:scale-105 flex items-center gap-2 mx-auto">Get Started Today <ArrowRight className="w-4 h-4" /></button>
          </div>
        </div>
      </section>
    </div>
