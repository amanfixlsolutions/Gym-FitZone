...
import {
  ...
  RefreshCw, Zap, MapPin, Search, Building2, Users,
} from "lucide-react";

// ── Steps ──────────────────────────────────────────────────────────
// 1 = enter email  →  2 = verify OTP  →  3 = select gym  →  4 = fill details  →  done
...
export default function SignupPage() {
  ...
  // ── Fetch gyms when reaching step 3 ───────────────────────────
  useEffect(() => {
    if (step !== 3) return;
    const fetchGyms = async () => {
      setGymsLoading(true);
      try {
        const res = await authAPI.getPublicGyms();
        setGyms(res.data || []);
      } catch {
        setGyms([]);
      } finally {
  ...
  // ── Step 3: Gym selected → go to details ──────────────────────
  ...
  const handleRegister = async (e) => {
    ...
    if (!selectedGym)           { setError("Please go back and select a gym."); return; }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-50 to-teal-50">
        <div className="text-center px-6">
          ...
          <p className="text-gray-500 text-sm mb-1">You've joined <strong>{selectedGym?.name}</strong></p>
          <p className="text-gray-400 text-xs">Account created successfully. Redirecting...</p>
  ...
  const STEP_LABELS = ["Email", "Verify OTP", "Select Gym", "Details"];
      ...
      <div className="hidden lg:flex lg:w-[42%] bg-gradient-to-br from-amber-500 via-orange-500 to-red-500 flex-col justify-between p-12 relative overflow-hidden flex-shrink-0">
        ...
        <div className="relative z-10">
          ...
          <p className="text-orange-100 text-base leading-relaxed mb-8">
            Choose your gym, create your account, and start training with world-class facilities.
          </p>
          <div className="space-y-3">
            {["Choose your preferred gym", "Access to all gym classes", "Personal trainer sessions", "Live Zoom fitness classes", "Progress tracking & analytics"].map((b, i) => (
        ...
        <div className="w-full max-w-md">
          ...
          {step === 1 && (
            <>
              ...
              <p className="text-sm text-gray-500 mb-7">Enter your email to get started. We'll send you a verification code.</p>
              {error && <div className="bg-red-50 border border-red-200 text-red-600 text-sm px-4 py-3 rounded-xl mb-5">{error}</div>}
              <form onSubmit={handleSendOTP} className="space-y-4">
                <div>
                  <label className="text-xs font-semibold block mb-1.5 uppercase tracking-wide text-gray-500">Email Address</label>
                ...
                <button type="submit" disabled={loading || !email.trim()}
                  ...
                  {loading ? <Loader2 size={16} className="animate-spin" /> : <><span>Send Verification Code</span><ArrowRight size={16} /></>}
                </button>
              </form>
              <p className="text-center text-sm text-gray-500 mt-6">Already have an account?{" "}<Link href="/login" className="text-amber-600 font-semibold hover:underline">Sign in</Link></p>
            </>
          )}
          ...
          {step === 2 && (
            <>
              ...
              <form onSubmit={handleVerifyOTP} className="space-y-5">
                <div>
                  <label className="text-xs font-semibold block mb-3 uppercase tracking-wide text-gray-500">Enter OTP</label>
                  <div className="flex gap-2 justify-between" onPaste={handleOtpPaste}>
                    {otp.map((digit, idx) => (
                      <input key={idx} ref={el => otpRefs.current[idx] = el} type="text" inputMode="numeric" maxLength={1} value={digit}
                        ...
                        className={`w-12 h-14 text-center text-xl font-black border-2 rounded-xl transition-all focus:outline-none ${digit ? "border-amber-500 bg-amber-50 text-amber-700" : "border-gray-200 bg-gray-50 text-gray-800 focus:border-amber-400"}`} />
                    ))}
                  </div>
                </div>
                <button type="submit" disabled={loading || otp.join("").length < OTP_LENGTH}
                  className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 disabled:opacity-60 text-white font-bold py-3 rounded-xl transition-all shadow-sm text-sm">
                  {loading ? <Loader2 size={16} className="animate-spin" /> : <><ShieldCheck size={16} /><span>Verify OTP</span></>}
                </button>
              </form>
              <div className="flex items-center justify-between mt-5">
                <button onClick={() => { setStep(1); setOtp(Array(OTP_LENGTH).fill("")); setError(""); }} className="text-xs text-gray-400 hover:text-gray-600 transition-colors">← Change email</button>
                <button onClick={handleResend} disabled={resendSecs > 0 || loading}
          ...
          {step === 3 && (
            <>
              ...
              <p className="text-sm text-gray-500 mb-5">Select the gym you want to join. You'll be registered as a member there.</p>
              {error && <div className="bg-red-50 border border-red-200 text-red-600 text-sm px-4 py-3 rounded-xl mb-4">{error}</div>}

              {/* Search */}
              <div className="relative mb-4">
                <Search size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" />
                <input type="text" placeholder="Search by gym name or city..." value={gymSearch} onChange={e => setGymSearch(e.target.value)}
              ...
              {gymsLoading ? (
                ...
                <div className="text-center py-10">
                  ...
                  <p className="text-sm text-gray-400 font-medium">No gyms found</p>
                  <p className="text-xs text-gray-300 mt-1">Try a different search term</p>
                </div>
              ) : (
                <div className="space-y-2.5 max-h-80 overflow-y-auto pr-1">
                  {filteredGyms.map(gym => (
                    <button key={gym._id} onClick={() => handleGymSelect(gym)}
                      className="w-full flex items-center gap-3 p-3.5 border-2 border-gray-100 rounded-2xl hover:border-amber-400 hover:bg-amber-50 transition-all text-left group">
                      {/* Logo / Avatar */}
                      <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center flex-shrink-0 overflow-hidden shadow-sm">
                        {gym.logo ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-gray-800 truncate group-hover:text-amber-700">{gym.name}</p>
                        <div className="flex items-center gap-1.5 mt-0.5">
                          <MapPin size={11} className="text-gray-400 flex-shrink-0" />
                          <p className="text-xs text-gray-500 truncate">{gym.city}</p>
                        </div>
                        {gym.totalMembers > 0 && (
                          <div className="flex items-center gap-1 mt-0.5">
                            <Users size={10} className="text-emerald-500" />
                            <p className="text-[10px] text-emerald-600 font-semibold">{gym.totalMembers} members</p>
                          </div>
                        )}
                      </div>
              ...
              <button onClick={() => { setStep(2); setError(""); }} className="text-xs text-gray-400 hover:text-gray-600 transition-colors mt-4 block">← Back to OTP</button>
            </>
          )}
          ...
          {step === 4 && (
            <>
              ...
              <p className="text-sm text-gray-500 mb-5">Fill in your details to complete registration.</p>

              {/* Selected gym badge */}
              {selectedGym && (
                <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-5">
                  <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center flex-shrink-0 overflow-hidden">
                    {selectedGym.logo ? <img src={selectedGym.logo} alt={selectedGym.name} className="w-full h-full object-cover" /> : <Dumbbell size={16} className="text-white" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] text-amber-600 font-semibold uppercase tracking-wide">Joining</p>
                    <p className="text-sm font-bold text-gray-800 truncate">{selectedGym.name}</p>
                    {selectedGym.city && <p className="text-xs text-gray-500">{selectedGym.city}</p>}
                  </div>
                  <button onClick={() => { setStep(3); setSelectedGym(null); setError(""); }}
                    className="text-[10px] text-amber-600 font-semibold hover:underline flex-shrink-0">Change</button>
                </div>
              )}
              ...
              <form onSubmit={handleRegister} className="space-y-4">
                {/* Name */}
                <div>
                  <label className="text-xs font-semibold block mb-1.5 uppercase tracking-wide text-gray-500">Full Name <span className="text-red-500">*</span></label>
                  <div className="relative">
                    <User size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" />
                    <input type="text" placeholder="Rahul Sharma" value={name} onChange={e => setName(e.target.value)} required autoFocus
                      className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl text-sm bg-gray-50 text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all" />
                  </div>
                </div>
                ...
                {/* Phone */}
                <div>
                  <label className="text-xs font-semibold block mb-1.5 uppercase tracking-wide text-gray-500">Phone Number <span className="text-gray-400 font-normal">(optional)</span></label>
                  <div className="relative">
                ...
                {/* Password */}
                <div>
                  <label className="text-xs font-semibold block mb-1.5 uppercase tracking-wide text-gray-500">Password <span className="text-red-500">*</span></label>
                  <div className="relative">
                  ...
                  {pwStrength && (
                    <div className="mt-2">
                      ...
                      <p className={`text-[10px] mt-1 font-semibold ${pwStrength.label === "Weak" ? "text-red-500" : pwStrength.label === "Fair" ? "text-amber-500" : pwStrength.label === "Good" ? "text-blue-500" : "text-emerald-500"}`}>{pwStrength.label} password</p>
                    </div>
                  )}
                </div>
                ...
                {/* Confirm Password */}
                <div>
                  <label className="text-xs font-semibold block mb-1.5 uppercase tracking-wide text-gray-500">Confirm Password <span className="text-red-500">*</span></label>
                  <div className="relative">
                    <Lock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" />
                    <input type={showCPw ? "text" : "password"} placeholder="Re-enter password" value={confirmPw} onChange={e => setConfirmPw(e.target.value)} required
                      className={`w-full pl-10 pr-11 py-3 border rounded-xl text-sm bg-gray-50 text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500/20 transition-all ${confirmPw && password !== confirmPw ? "border-red-400 focus:border-red-400" : confirmPw && password === confirmPw ? "border-emerald-400 focus:border-emerald-400" : "border-gray-200 focus:border-amber-500"}`} />
                    <button type="button" onClick={() => setShowCPw(v => !v)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                      {showCPw ? <EyeOff size={15} /> : <Eye size={15} />}
                    </button>
                    {confirmPw && password === confirmPw && <CheckCircle size={15} className="absolute right-10 top-1/2 -translate-y-1/2 text-emerald-500" />}
                  </div>
                  {confirmPw && password !== confirmPw && <p className="text-[10px] text-red-500 mt-1 font-medium">Passwords do not match</p>}
                </div>
                ...
                <button type="submit" disabled={loading || !name.trim() || password.length < 6 || password !== confirmPw}
                  ...
                  {loading ? <Loader2 size={16} className="animate-spin" /> : <><span>Create My Account</span><ArrowRight size={16} /></>}
                </button>
