...
// CheckinForm
// Workflow:
//   1. Gym Owner generates QR on Attendance page
//      → QR encodes: /checkin?gym=<gymId>&name=<gymName>
//   2. Member scans QR with phone camera
//      → Browser opens this page with gymId in URL
//   3. Member enters their registered phone number
//   4. We POST { gymId, phone } to /api/attendance/qr-checkin
//   5. Backend finds member by phone in that gym → marks attendance
// ─────────────────────────────────────────────────────────────────
function CheckinForm() {
  ...
  // ── Guard: gymId must be a 24-char hex ObjectId ────────────────
  const isValidGymId = /^[a-f\d]{24}$/i.test(gymId);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/attendance/qr-checkin`, {
        ...
        body:    JSON.stringify({ gymId, phone: rawPhone }),
      });
      ...
      <div className="min-h-screen bg-gradient-to-br from-red-600 to-rose-700 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 text-center">
          ...
          <p className="text-sm text-gray-500 leading-relaxed">
            This QR code is missing or has an invalid gym ID.
            Please ask your gym to regenerate the attendance QR code from the Attendance page.
          </p>
        </div>
      </div>
    );
        ...
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 text-center">
          <div
            className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-5 ${
              result.alreadyIn ? "bg-amber-100" : "bg-emerald-100"
            }`}
          >
            <CheckCircle
              size={40}
              className={result.alreadyIn ? "text-amber-500" : "text-emerald-500"}
            />
          </div>
